...
|`/effort` | 调整思考深度:low/medium/high/xhigh/max |
...
- Effort四档说明
  | 级别       | 速度   | 智能程度           | 合适场景                                                     |
  | :--------- | :----- | :----------------- | :----------------------------------------------------------- |
  | **low**    | 最快 ⚡ | 最低               | 简单、机械的任务：加注释、小重构、格式调整、简单问答         |
  | **medium** | 较快   | 中等               | 日常编码辅助：实现常规函数、修复简单 bug、写单元测试         |
  | **high**   | 较慢   | 高（**默认级别**） | 大多数复杂任务：跨文件重构、有一定难度的逻辑实现、调试       |
  | **xhigh**  | 慢     | 很高               | 高难度问题：复杂算法、架构设计、严密安全审查、多步骤推理     |
  | **max**    | 最慢 🐢 | 最强 🧠             | 极端复杂的任务：理解大型代码库的深层逻辑、需要深度推理的棘手问题 |
...
- 这三档构成了一个严格的**规则引擎**，其优先级固定为 **Deny > Ask > Allow**，以保证 Deny 规则无法被任何 Allow 规则覆盖。权限检查遵循“首个匹配获胜”原则：
  - **检查 Deny 规则列表** → 匹配到即阻止。
  - **检查 Ask 规则列表** → 匹配到即询问用户。
  - **检查 Allow 规则列表** → 匹配到即自动放行。
  - **都不匹配** → 进入默认逻辑，通常表现为 Ask 行为。
